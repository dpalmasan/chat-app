...
RemoteMessageQueuePublisher::RemoteMessageQueuePublisher(std::string host, std::uint16_t port)
    : host_(std::move(host)), port_(port) {}

ChatMessage RemoteMessageQueuePublisher::PublishMessage(const ChatMessage& message) {
    const std::string request =
        "PUBLISH|" + UrlEncode(message.message_from) +
...
std::string RemoteMessageQueuePublisher::SendRequest(const std::string& request) {
    const int sock = socket(AF_INET, SOCK_STREAM, 0);
    if (sock < 0) {
        throw std::runtime_error("failed to create message queue socket");
    }
    ...
    if (inet_pton(AF_INET, host_.c_str(), &addr.sin_addr) <= 0) {
        close(sock);
        throw std::invalid_argument("invalid message queue host");
    }

    if (connect(sock, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0) {
        close(sock);
        throw std::runtime_error("failed to connect to message queue worker");
    }
    ...
    if (line.rfind("ERR|", 0) == 0) {
        throw std::runtime_error("message queue error: " + line.substr(4));
    }
    throw std::runtime_error("invalid message queue response");
}

std::string RemoteMessageQueuePublisher::UrlEncode(const std::string& input) {
...
std::string RemoteMessageQueuePublisher::UrlDecode(const std::string& input) {
    std::string decoded;
...
std::vector<std::string> RemoteMessageQueuePublisher::Split(const std::string& line, char delim) {
...
ChatMessage RemoteMessageQueuePublisher::ParseMessageLine(const std::string& line) {
