...
RemoteDbMessageStore::RemoteDbMessageStore(std::string host, std::uint16_t port)
    : host_(std::move(host)), port_(port) {}

ChatMessage RemoteDbMessageStore::SaveMessage(ChatMessage message) {
...
std::vector<ChatMessage> RemoteDbMessageStore::LoadPendingMessagesFor(const UserId& recipient_id) {
...
void RemoteDbMessageStore::MarkMessageDelivered(const UserId& recipient_id, MessageId message_id) {
    const std::string request =
...
std::string RemoteDbMessageStore::SendRequest(const std::string& request) {
    ...
    if (getaddrinfo(host_.c_str(), port_text.c_str(), &hints, &result) != 0 || result == nullptr) {
        throw std::runtime_error("failed to resolve db host");
    }
    ...
    if (sock < 0) {
        throw std::runtime_error("failed to connect to db service");
    }
    ...
    if (line.rfind("ERR|", 0) == 0) {
        throw std::runtime_error("db service error: " + line.substr(4));
    }
    throw std::runtime_error("invalid db service response");
}

std::string RemoteDbMessageStore::UrlEncode(const std::string& input) {
...
std::string RemoteDbMessageStore::UrlDecode(const std::string& input) {
    std::string decoded;
...
std::vector<std::string> RemoteDbMessageStore::Split(const std::string& line, char delim) {
...
ChatMessage RemoteDbMessageStore::ParseMessageLine(const std::string& line) {
